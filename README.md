[![](https://github.com/mchmarny/sbomer/actions/workflows/on-push.yaml/badge.svg?branch=main)](https://github.com/mchmarny/sbomer/actions/workflows/on-push.yaml)
...
Utility for bulk image, license, package, and vulnerability sbomervery in containerize workloads on GCP.
...
* sbomerver currently deployed container images in Cloud Run, GCF, and GKE
...
* Available as CLI os Service (for continuous sbomervery)

![](docs/img/sbomer.gif)

Additionally, when deployed as a service, `sbomer` will:
...
`sbomer` provides an easy way to `sbomer`ver which of these container images are currently deployed, and automates the vulnerability/license scanning. 

## Install

You can use `sbomer` either as CLI or Service:
...
```shell
sbomer command [command options] [arguments...]
```

> You can use the `--help` flag on any level to get more information about the runtime, commands, of `sbomer` itself.

#### Images

sbomerver deployed images from specific runtime. To see all of the commands available for `img` (or `image`):

```shell
sbomer image --help
```

To sbomerver container images currently deployed in all of the supported runtimes:
...
* `--project` - scope sbomervery to a single project using project ID
...
sbomerver potential vulnerabilities in container images. To see all of the commands available for `vul` (or `vulnerability`):

```shell
sbomer vulnerability --help
```

Options: 

* `--file` - image list input file path to serve as a source (instead of sbomervery) (e.g. `sbomer img --output images.json`)
...
* `--format` - output format (`yaml` or `json` which is the default)
* `--project` - during sbomervery, runs only on specific project (project ID)
...
sbomerver licenses for OS and packages used in container images. To see all of the commands available for `lic` or `license`:
...
sbomerver packages used in container images. To see all of the commands available for `pkg` (or `packages`):

```shell
sbomer packages --help
```
...
When running as a service, `sbomer` automatically exports metrics and report data:

#### Metrics

`sbomer` metrics can be found in [Metric Explorer](https://cloud.google.com/monitoring/charts/metrics-explorer)

![](docs/img/metric-explore.png)

Custom time-series metrics created by `sbomer`:

* `sbomer/vulnerability/image` - count of images scanned for vulnerability (labels: project, version)
* `sbomer/vulnerability/severity` - vulnerability severity count (labels: project, version, kind)
* `sbomer/license/image` - count of images scanned for licenses (labels: project, version)
* `sbomer/license/count` - count of licenses (labels: project, version)
* `sbomer/package/image` - count of images scanned for packages (labels: project, version)
* `sbomer/package/count` - count of packages (labels: project, version)
...
`sbomer` service automatically exports its data to two BigQuery tables

Common elements:

* `batch_id` is the unique ID of each sbomervery operation 
